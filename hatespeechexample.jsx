// Importing React and various Material-UI components
...
function HateSpeechDetectionView() {
    ...
    const [tabIndex, setTabIndex] = useState(0); // Controls which tab is currently active
    const [modelname, setModelname] = useState(''); // Stores the name of the model entered by the user
    const [file, setFile] = useState(null); // Stores the uploaded file
    const [results, setResults] = useState(null); // Stores results received from the server
    ...
    // Handles changes in the Tab component
    const handleTabChange = (event, newValue) => {
        setTabIndex(newValue);
    };

    // Updates modelname state when the user types in the TextField
    const handleModelNameChange = (event) => {
        setModelname(event.target.value);
    };

    // Updates file state when the user selects a file
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setResults(data);
            setTabIndex(2); // Switch to the Results tab to display them
        } catch (err) {
